...
taskManagerModule.controller('expTrackerController', function ($scope,$http) {
	...
	function findAllTasks() {
		...
		success(function (data) {
			...
			$scope.selectedname = $scope.selection[0].personName
//			$scope.taskName="";
//			$scope.taskDesc="";
//			$scope.taskPriority="";
//			$scope.taskStatus="";
...
//	create a blank object to handle form data.
	...
	$scope.submitForm = function() {
		...
		for (var i = 0; i < $scope.persons.length; i++) {
			division_fact = division_fact + parseInt(document.getElementById("division"+$scope.persons[i].personName).value);

			if(division != " ")
				division = division+"-"+document.getElementById("division"+$scope.persons[i].personName).value;
			else
				division =  document.getElementById("division"+$scope.persons[i].personName).value;
		}
...
//		});
		...
		success(function (data) {
			if(data == 1)
			{
				//$("#messagebtn").toggleClass('btn btn-block btn-sucess btn-lg');


				$('#messagebtn').html("<font color='green'><b>Expense Added Successfully</b></font>");
